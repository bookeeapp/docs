...
description: >-
  Configure payment modes, retry logic, enrollment eligibility, and global trial
  pack limits in Kenko
---
...
\- \*\*Credit/Debit Card (External)\*\* (Default) - \*\*Cash\*\* (Default)
...
Enabling \*\*Pay Later\*\* lets members defer payments, which may lead to billing issues if not managed correctly. While booking a member from Kenko' platform into a Class or Appointment, Pay Later option will appear which can be later cleared off by the Member from the CRM or the Website and App.![Pay Later Option](../.gitbook/assets/Screenshot2025-04-24at11.47.19AM.png)The Outstanding payment will appear in the Member's profile under the \*\*Payments \\> Order\*\* Tab
...
New customers will be charged an enrollment fee upon purchase of a Subscription You can define a threshold (in months) after which returning customers are again considered eligible for enrollment fees.
...
This is different from Trial Membership toggle in Credit Packs and only applies to the global purchase restriction.![Global Trial Pack Setting](../.gitbook/assets/Screenshot2025-04-24at11.49.51AM.png)Learn more about Intro Packs here

## Best Practices

Only allow Pay Later for trusted members. Monitor dues regularly to avoid cashflow issues. Let Stripe optimize retry attempts to reduce failures. Avoid manual tweaks. Define location- or business-level limits on trial packs to prevent repeated use. Communicate eligibility and timing clearly to avoid customer confusion.
