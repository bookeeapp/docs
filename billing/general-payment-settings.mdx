...
<Accordion title="Pay Later" icon="clock">
  ...
  While booking a member from Kenko's platform into a Class or Appointment, Pay Later option will appear which can be later cleared off by the Member from the CRM or the Website and App.
...
Learn more about [Intro Packs](./intro-packs).
